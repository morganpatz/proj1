...
public class Security {
	 ...
	 * returns that value as a string. Returns the resulting value in a string
	 * if a value is found. Otherwise, an empty string is returned. Usage:
	 * --query: A string containing an SQL query. --conn: A connection to an SQL
	 * database.
	 */
	...
	/*
	 * A helper function for safely executing a single SQL update query. Returns
	 * 1 if the update is successful, -1 if an SQL error occured. Usage:
	 * --update: A string containing a proper SQL update statement. --conn: A
	 ...
	 * remove_friend. It returns the group_id of the group the user specified,
	 * encapsulated in a string, only if the user has permission to edit the
	 * group. Otherwise, an empty string is returned. Usage: --userid: the
	 * user_id of the user who requested this action --groupname: the name of
	 * the group specified by the user --friendid: the user_id of the one the
	 * user wishes to add or remove from the selected group. --conn: a
	 * connection to an sql database
	 */
	...
	/*
	 * A simple function that checks if the user has permission to edit a given
	 * image. Note that a user that can edit an image is also able to view it.
	 * Returns 1 if the user has permission, 0 if not. Usage: --username: the
	 * user_id of the user calling this function --photoid: the photo_id of the
	 * photo selected by the user --conn: a connection to an sql database
	 */
	...
	public int edit_allowed(String userid, String photoid, Connection conn) {
		...
		// If not, check if the user trying to view one of the user's own
		// images.
	...
	/*
	 * Creating a Group: Since one needs to be logged in to make a group, making
	 * one is easy. Well, almost. We need to generate a group id for the group
	 * first. Assume that: --userid is the user name of the user requesting the
	 * action --groupname is the name of the group submitted by the user, it
	 * DOES NOT have to be unique on its own, though the combination of user and
	 * group name as a tuple must be unique. if ((userid, groupname) is unique)
	 * { id += 1; INSERT INTO groups VALUES(id, userid, groupname, sysdate); }
	 * else { tell the user to pick a new group name }
	 * 
	 * For now, create_group returns an int. 1 if the operation succeeded, 0 if
	 * the insert was rejected.
	 * 
	...
	/*
	 * Updating a group's list of members: When adding a person into the group,
	 * the user needs to supply the user name of the friend being added, and the
	 * name of group which the user plans on adding this friend to. As group
	 * names alone are not unique, we need to figure out the group id from the
	 * supplied group name and the user id of the user performing the action.
	 * Assume that: --userid is the user name of the user requesting the action
	 * --groupname is the name of the group submitted by the user. --friendid is
	 * the user name of the friend the user has submitted. The query to find the
	 * correct id should be as follows: declare int gid = null; SELECT
	 ...
	 * date_added) VALUES(gid, friendid, sysdate); else {tell the user to try
	 * again}
	 * 
	 * The process for removing friends from a group list follows the same logic
	 * above, but concludes with an appropriate DELETE statement.
	 * 
	 ...
	 * 
	 * Usage: --userid: the user_id of the user who requested this action
	 * --groupname: the name of the group specified by the user --friendid: the
	 * user_id of the one the user wishes to add or remove from the selected
	 * group. --conn: a connection to an sql database
	 */
	...
	public int remove_friend(String userid, String groupname, String friendid,
		...
		// However, we still need to check if the friend is in the selected
		// group.
	...
	/*
	 * Viewing Images: A user may view an image if: --The value of PERMITTED is
	 * set to 1; that is, it's set to public --The user_id of the user is admin
	 * --The user_id of the user is the same as the one that uploaded the image
	 ...
	 * PERMITTED
	 * 
	 * If permission to view the image is granted, view_allowed returns 1. Else,
	 * it returns a 0.
	 */
	...
	/*
	 * Updating Image Information / Deleting Images: A user that can update a
	 * given image can also delete said image. In order to perform either on a
	 * given image, the user_id of the one requesting this action must be any of
	 * the following: --admin --the same user_id as the one that uploaded the
	 * image
	 */

	/*
	 * The function that edits an image if the user requesting to edit has
	 * permission to do so. It returns a 1 if the edit is successful; 0 if the
	 * request to edit the image is rejected. Usage: --username: the user_id of
	 * the user who requested this action --photoid: the photo_id of the
	 * selected photo --
	 */
